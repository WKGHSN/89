...
export default function ContactsPage() {
  const contact = useDataStore(s => s.contactInfo);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          <div className="lg:col-span-2 space-y-4">
            <div className="bg-white rounded-3xl shadow-soft p-6">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-lumi-muted text-sm">{contact.address}</p>
                  <a
                    href={`https://maps.google.com/?q=${contact.coordinates.lat},${contact.coordinates.lng}`}
                    target="_blank"
            ...
            <div className="bg-white rounded-3xl shadow-soft p-6">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <a href={`tel:${contact.phone.replace(/\s/g, '')}`} className="text-lumi-muted text-sm hover:text-lumi-rose transition-colors">
                    {contact.phone}
                  </a>
                </div>
              </div>
            ...
            <div className="bg-white rounded-3xl shadow-soft p-6">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <a href={`mailto:${contact.email}`} className="text-lumi-muted text-sm hover:text-lumi-rose transition-colors">
                    {contact.email}
                  </a>
                </div>
              </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <div className="space-y-1.5">
                    <p className="text-sm text-lumi-muted">{contact.workingHours.weekdays}</p>
                    <p className="text-sm text-lumi-muted">{contact.workingHours.saturday}</p>
                    <p className="text-sm text-lumi-muted">{contact.workingHours.sunday}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-3xl shadow-soft p-6">
              ...
              <div className="flex gap-3">
                <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 flex-1 px-4 py-3 bg-lumi-cream rounded-2xl hover:bg-lumi-blush/30 transition-colors">
                  <Instagram className="w-5 h-5 text-lumi-rose" />
                  <div>
                    <p className="text-xs text-lumi-muted">Instagram</p>
                    <p className="text-sm font-medium text-lumi-text">{contact.instagram}</p>
                  </div>
                </a>
                <a href="https://t.me" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 flex-1 px-4 py-3 bg-lumi-cream rounded-2xl hover:bg-lumi-blush/30 transition-colors">
                  <Send className="w-5 h-5 text-lumi-rose" />
                  <div>
                    <p className="text-xs text-lumi-muted">Telegram</p>
                    <p className="text-sm font-medium text-lumi-text">{contact.telegram}</p>
                  </div>
                </a>
              </div>
            </div>
          ...
          <div className="lg:col-span-3">
            <div className="bg-white rounded-3xl shadow-soft overflow-hidden h-full min-h-[500px]">
              <iframe
                src={`https://www.openstreetmap.org/export/embed.html?bbox=30.5134%2C50.4401%2C30.5334%2C50.4601&layer=mapnik&marker=${contact.coordinates.lat}%2C${contact.coordinates.lng}`}
