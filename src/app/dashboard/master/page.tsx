...
function getMasterIdForUser(userId: string): string | undefined {
  if (typeof window === 'undefined') return undefined;
  try {
    const raw = localStorage.getItem('lumibeauty-users');
    if (!raw) return undefined;
    const users = JSON.parse(raw) as { id: string; masterId?: string }[];
    return users.find(u => u.id === userId)?.masterId;
  } catch {
    return undefined;
  }
}
...
export default function MasterDashboard() {
  ...
  const masterId = getMasterIdForUser(user.id);
      ...
      <div className="page-container py-6">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
              ...
              <div className="flex flex-col items-center text-center gap-3 p-3 mb-3 border-b border-lumi-border">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1 justify-center mt-1">
                    <svg className="w-3 h-3 fill-amber-400" viewBox="0 0 20 20"><path d={STAR_PATH} /></svg>
          ...
          <div className="flex-1">
            ...
            {activeTab === 'bookings' && <BookingsListTab bookings={upcomingBookings} />}
...
function ScheduleTab({ master, bookings }: { master: Master; bookings: Booking[] }) {
  const now = new Date();
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white rounded-2xl shadow-soft p-5">
          ...
          <div className="grid grid-cols-7 gap-1">
            {days.map((day, idx) => {
              ...
              return (
                <button key={idx} onClick={() => setSelectedDate(dateStr)} className={cn('calendar-day relative', isSelected && 'selected', !isSelected && isToday && 'today')}>
                  {day}
        ...
        <div className="bg-white rounded-2xl shadow-soft p-5">
          <h3 className="font-medium text-lumi-text mb-4">{selectedDate ? formatDate(selectedDate) : 'Оберіть день'}</h3>
              ...
              {selectedBookings.map(b => (
                <div key={b.id} className="flex gap-3 p-3 bg-lumi-milk rounded-xl">
                  ...
                  <div>
                    ...
                    <span className={cn(getStatusBadgeClass(b.status), 'mt-1 inline-block text-xs')}>{getStatusLabel(b.status)}</span>
                  </div>
                </div>
              ))}
            </div>
...
function BookingsListTab({ bookings }: { bookings: Booking[] }) {
  return (
    <div>
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-sm">
                    <p className="text-lumi-muted flex items-center gap-1.5"><User className="w-3.5 h-3.5" /> {b.clientName}</p>
                    <p className="text-lumi-muted">{b.clientPhone}</p>
                    <p className="text-lumi-muted flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5" /> {formatDate(b.date)}</p>
                    <p className="text-lumi-muted flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" /> {b.time}</p>
                  </div>
...
function MasterProfileTab({ master }: { master: Master }) {
  return (
    <div>
      ...
      <div className="bg-white rounded-2xl shadow-soft p-6">
        ...
        <div className="mt-4 pt-4 border-t border-lumi-border">
          ...
          <p className="text-sm text-lumi-text">
            {['Нд', 'Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб'].filter((_, i) => master.workingDays.includes(i)).join(', ')}
            {' · '} {master.workingHours.start}–{master.workingHours.end}
          </p>
        </div>
        <p className="text-xs text-lumi-muted mt-4 italic">Для зміни профілю зверніться до адміністратора</p>
      </div>
    </div>
  );
}
